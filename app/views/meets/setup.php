...
<div class="mt-6 border-b border-slate-200">
    <nav class="flex flex-wrap gap-1 -mb-px" id="setupTabs">
        <?php foreach (['points' => 'Points', 'disciplines' => 'Disciplines', 'categories' => 'Categories', 'events' => 'Events', 'instances' => 'Event Instances'] as $key => $label): ?>
...
<style>.btn-sm{padding:.35rem .7rem;font-size:.8rem}.setup-tab.active{color:#2563EB;border-color:#2563EB}</style>
<script>window.MEET = { id: <?= (int) $meet['id'] ?>, base: <?= json_encode(url('meets/' . (int) $meet['id'])) ?> };</script>
